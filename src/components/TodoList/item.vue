<template>
  <n-list-item
      @click="emiter.emit('openDetail', item.id)"
      @contextmenu="handleContextMenu"
      style="margin-bottom: 5px; padding: 12px"
  >
    <template #prefix>
      <n-button text @click.stop="update({ id: item.id, finished: !item.finished })">
        <template #icon>
    ...
    <template #suffix>
      <n-icon class="pointer" @click.stop="update({ id: item.id, star: !item.star })" :size="20">
    ...
    <div>
      ...
      <p class="tag">
        <n-space size="small">
          <NTag type="error" round size="small" v-if="item.endTime">
          ...
          <n-tag size="small" round v-if="item.oneDay && !isMyDay">我的一天</n-tag>
        </n-space>
      </p>
    </div>
  </n-list-item>
...
const options: any = computed(() => [
  ...
  },
  {
    ...
    icon: renderIcon(SunOne),
    disabled: true,
    show: !isMyDay.value && oneDay.value,
  },
  {
    label: "添加到'我的一天'",
    key: "add-my-day",
